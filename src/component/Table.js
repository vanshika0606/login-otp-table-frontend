...
const Table = (props) => {
   ...
   const navigate = useNavigate()
    ...
    const logout = async()=>{
     let succ,msg;
    await  fetch("https://login-otp-table-frontend.onrender.com/logout").then((res)=>{
      return res.json();
}).then(async(data)=>{
  succ=data.success;
  msg= data.message;
...
if(succ===true){
  ...
  toast.error(msg)
    ...
    const fetchData = async()=>{
          fetch("https://login-otp-table-frontend.onrender.com/allrows").then((res)=>{
                return res.json();
          }).then(async(data)=>{
            setRows(data.table)
  ...
  <tbody>
    { rows.map( (r,ind)=>{
       ...
       onClick={()=>{
        ...
        fetch('https://login-otp-table-frontend.onrender.com/deleterow/' +r._id, {
      method: 'DELETE',
    })
